...
import { normalizePortOverride } from "./normalizePortOverride.js";
import * as axios from "@evershop/evershop/lib/util/get";
import { pool } from "@evershop/evershop/lib/postgres";
import { toPriceOverride } from "./toPriceOverride.js";
    ...
    availableShippingMethods: async (
      ...
      methods = methods.filter((method) => {
        ...
        if (method.condition_type === "price") {
          return (
            toPriceOverride(method.min) <= cart.sub_total &&
            cart.sub_total <= toPriceOverride(method.max)
          );
        } else if (method.condition_type === "weight") {
          return (
            toPriceOverride(method.min) <= cart.total_weight &&
            cart.total_weight <= toPriceOverride(method.max)
          );
        } else {
          return false;
        }
      ...
      methods = await Promise.all(
        methods.map(async (method) => {
          if (method.calculate_api) {
            ...
            const port = normalizePortOverride();
            ...
            return {
              ...method,
              cost: toPriceOverride(jsonResponse.data.data.cost, true),
            };
            ...
            return {
              ...method,
              cost: toPriceOverride(cost.toString(), false),
            };
          } else if (method.price_based_cost) {
            const subTotal = toPriceOverride(cart.sub_total);
            const priceBasedCost = method.price_based_cost
              .map(({ min_price, cost }) => ({
                min_price: toPriceOverride(min_price),
                cost: toPriceOverride(cost),
              }))
            ...
            return {
              ...method,
              cost: toPriceOverride(method.cost.toString(), false),
            };
          }
        }),
      );
